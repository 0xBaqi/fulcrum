...
Run npm run execution:wallet in the existing repository and open http://127.0.0.1:4318 in the browser containing the wallet extension. Connect the intended account, prepare and simulate, review the fresh minimum and SOL cost, then explicitly authorize the transaction and approve it in the wallet. Approval has a 120-second ceiling and is additionally bounded by actual Solana blockhash validity. After signing, fresh market evidence must pass the unchanged 15-second freshness and material-change checks before broadcast. Expiry or material change requires a new preparation and signature. Never retry an uncertain broadcast as a new trade: check confirmation first.
