...
import {canonical,hash} from '../engine.mjs';
import {evaluateV4WithPythPro} from './engine-v4.mjs';
export function replayWithPythPro(bundle) {
  ensure(
    bundle?.snapshotSha256 === hash(bundle?.snapshot),
    'SNAPSHOT_HASH_MISMATCH'
  );

  const result = evaluateV4WithPythPro(bundle.snapshot);

  ensure(
    canonical(result) === canonical(bundle.result),
...
export function strictAsset(bundle){
 const result=replayWithPythPro(bundle);
...
export function materialGuard(analysis,comparison,finalQuote,now,policy=EXECUTION_POLICY){
 ...
 const current=evaluateV4WithPythPro({
  ...comparison.snapshot,
  asOfMs: now
});
