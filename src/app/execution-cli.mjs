...
import {executionPipeline,bundleSnapshot} from '../execution/pipeline.mjs';
...
if(args[0]==='replay'){
 const r=replayReceipt(JSON.parse(await readFile(args[1],'utf8')));
 ...
 // Preserve strict future-evidence rejection while allowing bounded
 // provider/local-clock skew to settle against actual elapsed time.
 const liveCollector=settledCollector(collect);

 const snapshot=await liveCollector({
  underlying:'TSLA',
  amountRaw:'1000000',
  env:{...process.env,PYTH_API_KEY:''}
 });

 const analysis=bundleSnapshot(snapshot);
 ...
 if(analysis.result.status!=='WINNER'){
  ...
  const pipeline=executionPipeline({
   ...
   persist:b=>writeFile(out,JSON.stringify(b,null,2))
  });

  const r=await pipeline.prepare(
   analysis,
   args.includes('--wallet')?value('--wallet'):null
  );
